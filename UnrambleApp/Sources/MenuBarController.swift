...
final class MenuBarController: NSObject, NSMenuDelegate {
    ...
    private func buildOnboardingMenu(for statusItem: NSStatusItem) {
        ...
        let hint = NSMenuItem(
            title: "Finish setting up Unramble",
            action: nil,
            keyEquivalent: ""
        )
    ...
    @objc private func contributeMicData() {
        ...
        Task {
            ...
            let body = """
                > See [Microphone compatibility](https://github.com/mrinalwadhwa/unramble/issues/2) for context on what this data is used for.
            ...
            var components = URLComponents(
                string: "https://github.com/mrinalwadhwa/unramble/issues/new")!
    ...
    @objc private func openLanguageIssue() {
        if let url = URL(string: "https://github.com/mrinalwadhwa/unramble/issues/1") {
            NSWorkspace.shared.open(url)
        }
    }
