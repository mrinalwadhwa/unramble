...
| OpenAI realtime | Complete | The client uses the current transcription-session WebSocket intent, streams 24 kHz PCM, requests the low-latency transcription profile, preserves server errors, collects partial/final events, and treats an empty final transcript as no speech. Deterministic and opt-in live-service tests pass. |
