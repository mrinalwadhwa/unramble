...
//
// Enable: touch /tmp/unramble-test-nemotron-baseline
// Run:
//   cd UnrambleKit && xcodebuild test -scheme UnrambleKit \
//        -destination 'platform=macOS' \
//        -only-testing:UnrambleKitTests/NemotronBaseline \
//        SWIFT_ACTIVE_COMPILATION_CONDITIONS='$(inherited) UNRAMBLE_MLX_TESTS'
//
// Output (tail -f to watch live):
//   /tmp/unramble-nemotron-baseline.log    — human-readable dump
//   /tmp/unramble-nemotron-baseline.jsonl  — one JSON record per file
...
struct NemotronBaseline {
    ...
    func replay() async throws {
        guard FileManager.default.fileExists(
            atPath: "/tmp/unramble-test-nemotron-baseline")
        else { return }
        ...
        let log = BaselineLogger(path: "/tmp/unramble-nemotron-baseline.log")
        let jsonl = BaselineLogger(path: "/tmp/unramble-nemotron-baseline.jsonl")
