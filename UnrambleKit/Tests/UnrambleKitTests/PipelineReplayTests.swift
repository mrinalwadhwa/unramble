...
//
//   touch /tmp/unramble-test-replay
//   cd UnrambleKit && xcodebuild test -scheme UnrambleKit \
//        -destination 'platform=macOS' \
//        -only-testing:UnrambleKitTests/PipelineReplay \
...
//   /tmp/unramble-replay.log   — raw STT and polished text per file
...
struct PipelineReplay {
    ...
    func replay() async throws {
        guard FileManager.default.fileExists(atPath: "/tmp/unramble-test-replay")
        else { return }
        ...
        let log = ReplayLog(path: "/tmp/unramble-replay.log")
