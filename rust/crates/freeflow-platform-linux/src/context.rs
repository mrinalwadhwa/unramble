...
use freeflow_core::{AppContext, AppContextProvider, FreeFlowError, Result};
use x11rb::{
...
use crate::{desktop_environment, x11};
...
impl LinuxAppContextProvider {
    ...
    fn current_x11_context() -> Result<AppContext> {
        ...
        let terminal_hint = format!("{app_class} {app_name}").to_ascii_lowercase();
        let is_terminal = [
            "terminal",
            "konsole",
            "alacritty",
        ...
        .iter()
        .any(|candidate| terminal_hint.contains(candidate));

        Ok(AppContext {
            active_window_id: Some(window),
            process_id,
            app_class,
            app_name,
            window_title,
...
impl AppContextProvider for LinuxAppContextProvider {
    async fn current_context(&self) -> Result<AppContext> {
        tokio::task::spawn_blocking(Self::current_x11_context)
            .await
